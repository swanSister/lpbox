...
import imagePopup from '../../components/mars/imagePopup.vue'
import MarsHeader from '../../components/mars/MarsHeader.vue'
import MarsFooter from '../../components/mars/MarsFooter.vue'
...
export default {
  ...
  methods: {
    ...
    listUpdate(){
      if(this.marsDataCopy.length > 0) this.emitter.emit('showMarsLoading')
      this.marsArray = this.marsArray.concat(this.marsDataCopy.splice(0,this.countDiff))
      this.$forceUpdate()

      var that = this           
      setTimeout(() => {
         that.emitter.emit('hideMarsLoading')
      },1000)
